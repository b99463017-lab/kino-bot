...
# .env dagi ADMIN_IDS — bular "asosiy adminlar" (👑), ular botdan hech qachon
# o'chirib bo'lmaydi (faqat .env faylini tahrirlash orqali o'zgartiriladi).
_admin_raw = os.getenv("ADMIN_IDS") or os.getenv("ADMIN_ID", "0")
ADMIN_IDS = {int(x) for x in re.split(r"[,\s]+", _admin_raw.strip()) if x.lstrip("-").isdigit()}

DB_NAME = os.getenv("DB_NAME", "kino_bot.db")
CATALOG_PAGE_SIZE = 12
BROADCAST_DELAY = 0.05  # so'rovlar orasidagi tanaffus (Telegram flood-limitidan qochish uchun)
...
# ============================================================
#     XOTIRASI SAQLANADIGAN (PERSISTENT) FSM STORAGE — SQLite
# ============================================================
# aiogram'ning standart MemoryStorage'i bot qayta ishga tushirilganda
# (yoki kod yangilanganda / serverda qayta deploy qilinganda) barcha
# foydalanuvchi holatlarini (masalan admin nima kiritayotgani, foydalanuvchi
# qaysi serial bo'limida turgani) yo'qotib qo'yadi. Shu muammoni butunlay
# yechish uchun holatni bazaga (SQLite) yozib boradigan maxsus storage.
class SQLiteStorage(BaseStorage):
    ...
    def _key(storage_key: StorageKey) -> str:
        # StorageKey versiyadan versiyaga qo'shimcha maydonlar olishi mumkin,
        # shuning uchun mavjud barcha maydonlardan universal kalit yasaymiz.
        parts = {f.name: getattr(storage_key, f.name) for f in dataclasses.fields(storage_key)}
    ...
    async def set_state(self, key: StorageKey, state=None) -> None:
        ...
        k = self._key(key)
        await conn.execute(
            "INSERT INTO fsm_storage (key, state, data) VALUES (?, ?, COALESCE((SELECT data FROM fsm_storage WHERE key=?), '{}')) "
    ...
    async def get_state(self, key: StorageKey) -> str | None:
        conn = await self._get_conn()
        cur = await conn.execute("SELECT state FROM fsm_storage WHERE key=?", (self._key(key),))
    ...
    async def set_data(self, key: StorageKey, data) -> None:
        ...
        k = self._key(key)
        await conn.execute(
            "INSERT INTO fsm_storage (key, state, data) VALUES (?, (SELECT state FROM fsm_storage WHERE key=?), ?) "
            "ON CONFLICT(key) DO UPDATE SET data=excluded.data",
            (k, k, json.dumps(dict(data))),
        )
        await conn.commit()

    async def get_data(self, key: StorageKey) -> dict:
        conn = await self._get_conn()
        cur = await conn.execute("SELECT data FROM fsm_storage WHERE key=?", (self._key(key),))
        row = await cur.fetchone()
        if row and row[0]:
            try:
...
async def init_db() -> None:
    ...
    await db.execute(
        "CREATE TABLE IF NOT EXISTS channels (id INTEGER PRIMARY KEY AUTOINCREMENT, chat_id TEXT UNIQUE, link TEXT)"
    )
    await db.execute("CREATE TABLE IF NOT EXISTS movies (code TEXT PRIMARY KEY, title TEXT, file_id TEXT, added_at TEXT)")
    await db.execute(
        "CREATE TABLE IF NOT EXISTS series (id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT NOT NULL, "
        "title TEXT NOT NULL, section TEXT, episode TEXT, file_id TEXT NOT NULL, added_at TEXT)"
    )
    await db.execute("CREATE INDEX IF NOT EXISTS idx_series_code ON series(code)")
    await db.execute("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)")
    await db.execute(
        "CREATE TABLE IF NOT EXISTS admins (user_id INTEGER PRIMARY KEY, added_by INTEGER, "
        "added_at TEXT, is_owner INTEGER DEFAULT 0)"
    )
    await db.commit()
    ...
    # .env dagi asosiy adminlarni bazaga "owner" sifatida urug'lash
    for admin_id in ADMIN_IDS:
        await db.execute(
            "INSERT OR IGNORE INTO admins (user_id, added_by, added_at, is_owner) VALUES (?, NULL, ?, 1)",
            (admin_id, now_iso()),
        )
    await db.commit()
...
class IsAdmin(BaseFilter):
    async def __call__(self, event: Message | CallbackQuery) -> bool:
        if event.from_user is None:
            return False
        return await is_admin_user(event.from_user.id)
...
async def check_subscription(user_id: int) -> InlineKeyboardMarkup | None:
    ...
    for row in channels:
        ...
        try:
            ...
            logger.error("Obuna tekshirish xatosi (%s): %s", ch_id, e)
            unsubbed.append(ch_link)

    if unsubbed:
        btns = [
            [InlineKeyboardButton(text=f"📢 {i}-Kanalga obuna bo'lish", url=link)]
            for i, link in enumerate(unsubbed, start=1)
        ]
...
async def global_main_menu(message: Message, state: FSMContext):
    ...
    await message.answer("🏠 Bosh menyudasiz.", reply_markup=get_main_keyboard(admin))
...
async def cancel_handler(message: Message, state: FSMContext):
    await state.clear()
    admin = await is_admin_user(message.from_user.id)
    await message.answer("❌ Amal bekor qilindi.", reply_markup=get_main_keyboard(admin))
...
async def start_cmd(message: Message, command: CommandObject, state: FSMContext) -> None:
    ...
    if code:
        await process_search_code(message.chat.id, state, code)
    else:
        start_text = await get_setting("start")
        admin = await is_admin_user(message.from_user.id)
        await message.answer(start_text, reply_markup=get_main_keyboard(admin))
...
async def sub_check_callback(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if code:
        await process_search_code(call.message.chat.id, state, code)
    else:
        start_text = await get_setting("start")
        admin = await is_admin_user(call.from_user.id)
...
async def process_search_code(chat_id: int, state: FSMContext, code: str) -> None:
...
async def series_section_chosen(message: Message, state: FSMContext):
    ...
    if not episodes:
        cur2 = await db.execute(
            "SELECT code FROM movies WHERE code=? UNION SELECT code FROM series WHERE code=?",
            (section_name, section_name),
        )
        if await cur2.fetchone():
            await state.clear()
            return await process_search_code(message.chat.id, state, section_name)
...
async def back_to_sections_view(message: Message, state: FSMContext):
    data = await state.get_data()
    await process_search_code(message.chat.id, state, data["code"])
...
async def series_episode_chosen(message: Message, state: FSMContext):
    ...
    cur = await db.execute(
        "SELECT title, file_id FROM series WHERE code=? AND section=? AND episode=?", (code, section, episode_name)
    )
    ...
    if not series_data:
        ...
        if await cur2.fetchone():
            await state.clear()
            return await process_search_code(message.chat.id, state, episode_name)
    ...
    # State qasddan tozalanmaydi — foydalanuvchi keyingi qism tugmasini bosib davom etaveradi.
...
@router.message(F.text == "✍️ Matnlarni o'zgartirish", IsAdmin(), StateFilter(None))
...
async def admin_stats(message: Message) -> None:
    ...
    admins_cnt = (await (await db.execute("SELECT COUNT(*) c FROM admins")).fetchone())["c"]

    text = (
        f"📊 <b>Bot statistikasi</b>\n\n"
        f"👤 Foydalanuvchilar: <b>{users}</b>\n"
        f"🎬 Kinolar: <b>{movies}</b>\n"
        f"📺 Seriallar: <b>{s_titles}</b>\n"
        f"🎞 Jami serial qismlari: <b>{episodes}</b>\n"
        f"📢 Majburiy obunalar: <b>{channels}</b>\n"
        f"👥 Adminlar: <b>{admins_cnt}</b>"
    )
    await message.answer(text)


# --- MATNLARNI O'ZGARTIRISH ---
@router.message(F.text == "✍️ Start matni", IsAdmin(), StateFilter(None))
...
async def save_start_text(message: Message, state: FSMContext) -> None:
    await set_setting("start", message.html_text or message.text)
    await message.answer("✅ Start matni o'zgartirildi!", reply_markup=get_admin_main_kb())
    await state.clear()


@router.message(F.text == "✍️ Yordam matni", IsAdmin(), StateFilter(None))
...
async def save_help_text(message: Message, state: FSMContext) -> None:
    await set_setting("help", message.html_text or message.text)
    await message.answer("✅ Yordam matni o'zgartirildi!", reply_markup=get_admin_main_kb())
    await state.clear()


@router.message(F.text == "📸 Instagram havolasi", IsAdmin(), StateFilter(None))
async def edit_insta_prompt(message: Message, state: FSMContext) -> None:
    await message.answer(
        "📸 <b>Yangi Instagram havolasini yuboring:</b>\n<i>O'chirish uchun 'none' deb yozing.</i>",
        reply_markup=get_cancel_kb(True),
    )
...
async def save_insta_text(message: Message, state: FSMContext) -> None:
    ...
    if new_link.lower() != "none" and not re.match(r"^https?://", new_link):
        await message.answer("⚠️ Havola https:// bilan boshlanishi kerak (yoki 'none').")
        return
...
# --- ADMINLAR BOSHQARUVI ---
@router.message(F.text == "👥 Adminlar boshqaruvi", IsAdmin(), StateFilter(None))
async def admin_users_menu(message: Message):
    await message.answer("👥 Adminlarni boshqarish:", reply_markup=get_admin_users_kb())


@router.message(F.text == "📋 Adminlar ro'yxati", IsAdmin(), StateFilter(None))
async def list_admins(message: Message):
    cur = await db.execute("SELECT user_id, is_owner FROM admins ORDER BY is_owner DESC, user_id")
    rows = await cur.fetchall()
    text = "👥 <b>Adminlar ro'yxati:</b>\n\n"
    for r in rows:
        tag = " 👑 (asosiy, .env)" if r["is_owner"] else ""
...
async def ask_new_admin(message: Message, state: FSMContext):
    await message.answer(
        ...
        "<i>ID olish uchun o'sha foydalanuvchi botga <code>/id</code> buyrug'ini yuborishi kifoya.</i>",
        reply_markup=get_cancel_kb(True),
    )
    await state.set_state(AddAdmin.user_id)


@router.message(AddAdmin.user_id, IsAdmin())
async def save_new_admin(message: Message, state: FSMContext):
    ...
    new_id = int(text)
    await db.execute(
        "INSERT OR IGNORE INTO admins (user_id, added_by, added_at, is_owner) VALUES (?, ?, ?, 0)",
        (new_id, message.from_user.id, now_iso()),
    )
    await db.commit()
    await message.answer(f"✅ <code>{new_id}</code> endi admin!", reply_markup=get_admin_main_kb())
    await state.clear()
    try:
        await bot.send_message(new_id, "🎉 Sizga ushbu botda admin huquqi berildi! /start bosing.")
    except Exception:  # noqa: BLE001
        pass  # Foydalanuvchi botni hali ishga tushirmagan bo'lishi mumkin — bu xato emas.


@router.message(F.text == "➖ Admin o'chirish", IsAdmin(), StateFilter(None))
async def ask_del_admin(message: Message, state: FSMContext):
    await message.answer(
        "➖ O'chirmoqchi bo'lgan adminning ID raqamini yuboring.\n"
        "<i>(👑 belgili asosiy adminlarni faqat .env fayldan o'chirish mumkin)</i>",
        reply_markup=get_cancel_kb(True),
    )
    await state.set_state(DelAdmin.user_id)


@router.message(DelAdmin.user_id, IsAdmin())
async def save_del_admin(message: Message, state: FSMContext):
    ...
    if target in ADMIN_IDS:
        await message.answer("❌ Bu asosiy (👑) admin, uni faqat .env fayldan o'chirish mumkin.", reply_markup=get_admin_main_kb())
        await state.clear()
        return
    cur = await db.execute("SELECT 1 FROM admins WHERE user_id=?", (target,))
    if not await cur.fetchone():
        await message.answer("❌ Bunday admin topilmadi.", reply_markup=get_admin_main_kb())
        await state.clear()
        return
    await db.execute("DELETE FROM admins WHERE user_id=?", (target,))
    await db.commit()
    await message.answer(f"✅ <code>{target}</code> adminlikdan olindi.", reply_markup=get_admin_main_kb())
    await state.clear()


# --- KATALOG ---
@router.message(F.text == "🗂 Katalog (Baza)", IsAdmin(), StateFilter(None))
...
async def send_catalog_page(message: Message, page: int):
    ...
    cur = await db.execute(
        "SELECT DISTINCT code, title FROM series ORDER BY code LIMIT ? OFFSET ?", (CATALOG_PAGE_SIZE + 1, offset)
    )
    series = await cur.fetchall()
    text += "\n<b>📺 SERIALLAR:</b>\n"
    shown_series = series[:CATALOG_PAGE_SIZE]
    ...
    if len(movies) > CATALOG_PAGE_SIZE or len(series) > CATALOG_PAGE_SIZE:
        nav.append(KeyboardButton(text="Keyingi ➡️"))
...
# --- O'CHIRISH ---
@router.message(F.text == "🗑 O'chirish", IsAdmin(), StateFilter(None))
...
async def confirm_delete(message: Message, state: FSMContext) -> None:
    ...
    if not movie and not series:
        await message.answer(f"❌ <code>{html.escape(code)}</code> kodi topilmadi.")
        return
    ...
    kb = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="✅ Ha, o'chirish"), KeyboardButton(text="❌ Yo'q, bekor qilish")]],
        resize_keyboard=True,
    )
...
async def process_delete(message: Message, state: FSMContext) -> None:
    if message.text == "✅ Ha, o'chirish":
        ...
        await db.commit()
        await message.answer("✅ Ma'lumot tozalandi.", reply_markup=get_admin_main_kb())
    else:
...
async def ask_channel_link(message: Message, state: FSMContext) -> None:
    ...
    if message.forward_origin:
        if isinstance(message.forward_origin, MessageOriginChannel):
            chat_id = str(message.forward_origin.chat.id)
        elif isinstance(message.forward_origin, MessageOriginChat):
            chat_id = str(message.forward_origin.sender_chat.id)
    ...
    if not chat_id:
        ...
        if "t.me/" in text:
            ...
            await message.answer("⚠️ Iltimos, faqat ID raqam yoki @username yuboring. Yoki forward qiling:")
            return
...
async def save_channel(message: Message, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    await db.execute(
        "INSERT INTO channels (chat_id, link) VALUES (?, ?) ON CONFLICT(chat_id) DO UPDATE SET link=excluded.link",
        (data["chat_id"], link),
    )
    await db.commit()
...
@router.message(F.text == "➖ Kanal o'chirish", IsAdmin(), StateFilter(None))
async def del_channels_menu(message: Message, state: FSMContext) -> None:
    cur = await db.execute("SELECT id, chat_id FROM channels")
    channels = await cur.fetchall()
    if not channels:
        await message.answer("Bazada kanallar yo'q!", reply_markup=get_admin_main_kb())
        return

    text = "<b>Kanallar:</b>\n"
    for r in channels:
        text += f"ID: <code>{r['id']}</code> | {r['chat_id']}\n"
...
# --- KINO QO'SHISH ---
@router.message(F.text == "🎬 Kino qo'shish", IsAdmin(), StateFilter(None))
...
# --- SERIAL QO'SHISH ---
@router.message(F.text == "📺 Serial qo'shish", IsAdmin(), StateFilter(None))
...
async def process_series_code_add(message: Message, state: FSMContext) -> None:
    ...
    if existing:
        ...
        sections = await (
            await db.execute("SELECT DISTINCT section FROM series WHERE code=? AND section IS NOT NULL", (code,))
        ).fetchall()
        btns = [[KeyboardButton(text=row["section"])] for row in sections]
...
async def process_series_video(message: Message, state: FSMContext) -> None:
    ...
    await db.commit()
    await message.answer("✅ Serial qismi saqlandi!", reply_markup=get_admin_main_kb())
    await state.clear()


# --- BROADCAST ---
@router.message(F.text == "📣 Habar yuborish", IsAdmin(), StateFilter(None))
...
async def preview_broadcast(message: Message, state: FSMContext) -> None:
    ...
    kb = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="✅ Yuborish"), KeyboardButton(text="❌ Bekor qilish")]], resize_keyboard=True
    )
...
async def main() -> None:
    ...
    await bot.set_my_commands(
        [
            ...
            BotCommand(command="id", description="Telegram ID raqamingizni ko'rish"),
        ]
    )
    logger.info("Bot ishga tushdi. Adminlar (env): %s", ADMIN_IDS)
    try:
